...
const EditTalentModal = ({ isOpen, onClose, onSuccess, talent }) => {
  ...
  // Populate form when talent changes
  useEffect(() => {
    if (talent && isOpen) {
      setFormData({
        name: talent.name || '',
        description: talent.bio || talent.detail || '',
        rate: talent.charges && talent.charges !== '—' 
          ? talent.charges.replace(/[^0-9.]/g, '') 
          : '',
        rating: talent.rating !== '—' && talent.rating !== undefined && talent.rating !== null 
          ? talent.rating.toString() 
          : '',
        reviews: talent.reviews !== '—' && talent.reviews !== undefined && talent.reviews !== null 
          ? talent.reviews.toString() 
          : '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/talents/${talent.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(detail.detail || 'Failed to update talent')
      }

      const updatedTalent = await response.json()
      onSuccess(updatedTalent)
      onClose()
    } catch (err) {
      logError('Failed to update talent', err)
      ...
      >
        <div className="p-8">
          ...
          <div className="flex items-start justify-between mb-6">
            <h2 className="text-2xl font-bold text-gray-900">Edit Talent</h2>
            <button
              onClick={handleClose}
