...
import { Play, Square, RefreshCw, AlertCircle, CheckCircle, DollarSign, Target, Clock } from 'lucide-react'
...
const GuruAutoBid = () => {
  ...
  const [stats, setStats] = useState({ bids_today: 0, bids_week: 0, success_week: 0, is_running: false })
  ...
  const fetchLogs = async () => {
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`${API_URL}/api/guru/autobid/history?limit=20`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (response.ok) {
        const data = await response.json()
  ...
  const toggleAutoBid = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const endpoint = stats.is_running ? '/api/guru/autobid/stop' : '/api/guru/autobid/start'
      const response = await fetch(`${API_URL}${endpoint}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (response.ok) {
        toast.success(stats.is_running ? 'Guru auto-bid stopped' : 'Guru auto-bid started')
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Guru Auto Quote</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">Automated quote submission</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => { fetchStats(); fetchLogs() }} className="p-2 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
            <RefreshCw size={15} />
          </button>
          <button
            ...
            style={{ backgroundColor: stats.is_running ? '#ef4444' : GURU_COLOR }}
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {[
          { label: 'Quotes Today', value: stats.bids_today, icon: Target },
          { label: 'Quotes This Week', value: stats.bids_week, icon: DollarSign },
      ...
      <div className={`flex items-center gap-3 p-4 rounded-xl border ${
        stats.is_running ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800' : 'bg-gray-50 dark:bg-gray-800 border-gray-200 dark:border-gray-700'
      }`}>
        <div className={`w-2.5 h-2.5 rounded-full ${stats.is_running ? 'animate-pulse' : 'bg-gray-400'}`} style={stats.is_running ? { backgroundColor: GURU_COLOR } : {}} />
        <p className={`text-sm font-medium ${stats.is_running ? 'text-orange-800 dark:text-orange-300' : 'text-gray-600 dark:text-gray-400'}`}>
          {stats.is_running ? 'Auto-quoter is running' : 'Auto-quoter is stopped'}
        </p>
      </div>
      ...
      <div>
        ...
        {isLoading && <div className="flex justify-center py-8"><img src={gif} alt="Loading" className="w-10 h-10 object-contain" /></div>}
        {!isLoading && logs.length === 0 && (
          <div className="text-center py-10 text-gray-500 dark:text-gray-400">
            <Clock size={32} className="mx-auto mb-2 opacity-30" />
            <p className="text-sm">No quote activity yet</p>
          </div>
        )}
